...
# ==============================================================================
st.set_page_config(page_title="Ultimate Quant & Derivatives Terminal V9.4", page_icon="🏛️", layout="wide", initial_sidebar_state="collapsed")
...
.stTabs [data-baseweb="tab-list"] { gap: 6px; background-color: rgba(15, 23, 42, 0.6); border-radius: 12px; padding: 10px; border: 1px solid rgba(56, 189, 248, 0.2); }
.stTabs [data-baseweb="tab"] { color: #94a3b8 !important; font-weight: 600 !important; border-radius: 8px !important; padding: 10px 16px !important; font-size: 12px !important; transition: all 0.3s ease; }
...
TEXTS = {
    "zh": {
        ...
        "subtitle": "融合 DCF、敏感性热力图、动态 NWC、LBO、同业 Comps、蒙特卡洛、有效前沿与 Black-Scholes 期权波动率微笑",
        "quick_tag": "🔥 热门快捷测评：",
    ...
    "en": {
        ...
        "subtitle": "Integrated DCF, Sensitivity Heatmap, Hardcore NWC, LBO, Comps, Monte Carlo, Efficient Frontier & Black-Scholes Volatility Smile",
...
# ==============================================================================
# 3. 旗舰级全功能量化与 PE 引擎 (增加 Black-Scholes 与期权定价函数)
# ==============================================================================
...
def main():
    ...
    if ticker_input:
        with st.spinner("Compiling Quant & Derivatives Terminal Engine..."):
            ...
            # ==========================================
            # TAB 7: 📉 期权链与隐含波动率微笑 (Volatility Smile & Greeks)
            # ==========================================
            with tab7:
                ...
                try:
                    ...
                    if exp_dates:
                        ...
                        opt_tab1, opt_tab2 = st.tabs(["📈 Implied Volatility Smile (波动率微笑)", "🛡️ Black-Scholes & Greeks Table (希腊字母矩阵)"])
                        
                        with opt_tab1:
                            st.markdown(f"**Volatility Smile / Skew for Expiry: {selected_expiry} (T = {T_days} days)**")
                            fig_smile = go.Figure()
                            ...
                            st.info("💡 **波动率微笑 / 斜面 (Volatility Smile / Skew) 解读**：在实际市场中，虚值看跌期权 (OTM Puts) 的隐含波动率往往高于平值或看涨期权，形成向左上倾斜的‘微笑曲线’，反映了机构对尾部黑天鹅下跌风险的避险买盘需求。")

                        with opt_tab2:
                            st.markdown("**Live Option Chain with Black-Scholes Theoretical Pricing & Greeks**")
                            if not calls.empty:
                                ...
                                for _, row in sample_calls.iterrows():
                                    ...
                                    bs_results.append({
                                        "Strike": K, "Market Price": row['lastPrice'], "BS Fair Price": round(bs_p, 2),
                                        "IV (%)": round(iv * 100, 1), "Delta": round(delta, 2), "Gamma": round(gamma, 3),
                                        "Theta": round(theta, 2), "Vega": round(vega, 2)
                                    })
                                bs_df = pd.DataFrame(bs_results)
                                st.dataframe(bs_df, use_container_width=True)
                                st.caption("✨ *注：Delta 衡量股价变动 1$ 带来的期权变动；Gamma 衡量 Delta 加速度；Theta 衡量每日时间价值损耗；Vega 衡量波动率每变动 1% 的期权盈亏。*")
                    else:
    ...
    with st.container(border=True):
        ...
        st.markdown("<div style='text-align: center; color: #94a3b8; font-size: 12px; border-top: 1px solid rgba(255,255,255,0.06); padding-top: 10px;'>© 2026 Thomas. All rights reserved. | Flagship Quant & Derivatives Workstation V9.4.</div>", unsafe_allow_html=True)
