...
def draw_beta_scatter(engine):
    ...
    fig.add_trace(go.Scatter(x=x_range, y=engine.beta * x_range, mode='lines', line=dict(color='#38bdf8', width=2), name='Fit Line'))
    ...
    if ticker_input:
        with st.spinner("Initializing Deep Valuation Engine..."):
            ...
            if val > 0 and engine.price > 0:
                ...
                # --- Row 2: Lie Detector & Rating (并排等高卡片) ---
                ...
                with col_lie:
                    with st.container(border=True):
                        ...
                        st.markdown(f"Market Implied Growth Rate: <b style='color:#38bdf8; font-size:24px;'>{implied_g_str}</b>", unsafe_allow_html=True)
